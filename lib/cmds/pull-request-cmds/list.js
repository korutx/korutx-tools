...
exports.builder = yargs => yargs
    .option('project-type', {
        describe: 'tipo de proyecto que se desea listar',
        type: 'string',
        required: true
    })
...
exports.handler = async ({ projectType }) => {
    try {
        ...
        let projects
        if(projectType === 'elm-module'){
            projects = await getModules(conf, team, bitbucket)
        }
        
        projects.values.forEach(async p => {
            let { data, headers } = await bitbucket.repositories.listPullRequests({ 
                repo_slug: p.slug,
                state: 'OPEN', 
                username: team
            })
            if(data.values.length > 0){
                data.values.forEach(pr => {
                    console.log(`id: ${p.slug}/${pr.id}`)    
                    console.log(`title: ${pr.title}`)    
                    console.log(`author: ${pr.author.nickname}`)    
                    console.log(`---`)        
                })
