...
exports.command = 'reject <issueId>'
...
exports.handler = async ({ issueId }) => {
    try {
        ...
        const { data, headers } = await bitbucket.pullrequests.create(
            { _body: {
                title: `Issue: https://${jiraHost}/browse/${issueId} ${issueId} #qa #comment Ticket listo para pruebas funcionales`,
                source: { branch: { name: `feature-${issueId}` }},
                destination: { branch: { name: 'master' /*"development"*/ }},
                reviewers,
